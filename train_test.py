...
def train():
    ...
    for batch_idx, train_batch in tqdm(enumerate(training_data), total=len(training_data)):
        ...
        if config.training.mixup:
            ...
            optimizer.step()
        if config.encoder.model == 'VGGish':
            y_hat = model(f_names, tgt, target_padding_mask=tgt_pad_mask)
        else:
...
parser.add_argument('-n', '--exp_name', type=str, default='exp_filtered', help='name of the experiment')
parser.add_argument('-m', '--mask_type', type=str, default='zero_value', help='masking type')

args = parser.parse_args()

config = get_config('settings_test.yaml')
